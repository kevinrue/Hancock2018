...
To do so, we store manually curated cell type annotations (obtained from the [Guided Clustering Tutorial](https://satijalab.org/seurat/v3.0/pbmc3k_tutorial.html) of the `r CRANpkg("Seurat")` package) in the `"seurat.celltype"` cell metadata.
...
signatures <- learnSignatures(
    ...
    threshold = 2, n = 2, min.diff = 0.2, diff.method = "min", min.prop = 0.1)
x <- as.data.frame(signatures)
x <- cbind(x, Symbol_TENx=rowData(sce)[x$element, "Symbol_TENx"])
kable(x)
```

Note that the table above does include some of the markers suggested in the [Seurat Guided Clustering Tutorial](https://satijalab.org/seurat/v3.0/pbmc3k_tutorial.html) (listed below).
...
For reference, the cell type identity associated with markers mentioned in the [Guided Clustering Tutorial](https://satijalab.org/seurat/v3.0/pbmc3k_tutorial.html) is indicated in each panel subtitle, where applicable. 
...
<!-- The following chunk fails after the DelayedArray update to 0.11.4 -->
...
scree0 <- makeMarkerProportionScree(as.matrix(detection0))
# Plot
