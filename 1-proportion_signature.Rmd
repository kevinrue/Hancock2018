...
gsc <- GeneSetCollection(list(
    GeneSet(setName="CD4 T cells", c("IL7R"), collectionType=NullCollection()),
    GeneSet(setName="CD14+ Monocytes", c("CD14", "LYZ"), collectionType=NullCollection()),
    GeneSet(setName="B cells", c("MS4A1"), collectionType=NullCollection()),
    GeneSet(setName="CD8 T cells", c("CD8A"), collectionType=NullCollection()),
    GeneSet(setName="FCGR3A+ Monocytes", c("FCGR3A", "MS4A7"), collectionType=NullCollection()),
    GeneSet(setName="NK cells", c("GNLY", "NKG7"), collectionType=NullCollection()),
    GeneSet(setName="Dendritic Cells", c("FCER1A", "CST3"), collectionType=NullCollection()),
    GeneSet(setName="Megakaryocytes", c("PPBP"), collectionType=NullCollection())
))
gsc
```
...
```{r}
markers_unique <- unique(unlist(geneIds(gsc)))
...
knitr::kable(head(markerDetectionMatrix))
```
...
filts <- lapply(
    names(gsc),
    function(x){ parse(text=paste(geneIds(gsc[[x]]), collapse="&")) }
)
names(filts) <- names(gsc)
...
colnames(proportionSignatureByCluster) <- paste(
    ...
    sprintf("(%s)", lapply(gsc, function(x){paste(geneIds(x), collapse=", ")})),
    sep="\n"
)
