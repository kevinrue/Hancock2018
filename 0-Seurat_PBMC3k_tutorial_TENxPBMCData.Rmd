...
The first time the following code chunk is run may take longer, as the data is downloaded from the web and cached locally; subsequent evaluations of the code chunk should only take a few seconds as the data set is then loaded from the local cache.

```{r}
library(TENxPBMCData)
tenx_pbmc3k <- TENxPBMCData(dataset="pbmc3k")
colnames(tenx_pbmc3k) <- paste0("Cell", seq_len(ncol(pbmc.data)))
tenx_pbmc3k
```

# Preparing the data

Next, we prepare a sparse matrix that emulates the first section of the [Guided Clustering Tutorial](https://satijalab.org/seurat/pbmc3k_tutorial_1_4.html).
...
Save the original `SingleCellExperiment` object, after adding the cluster assignments.

```{r}
colData(tenx_pbmc3k)[pbmc@cell.names, "seurat.ident"] <- pbmc@ident
tenx_pbmc3k$seurat.ident <- factor(tenx_pbmc3k$seurat.ident, levels(pbmc@ident))
...
It will be included in the `hancock` package, and mapped using the dummy cell names defined at the start of this notebook.
